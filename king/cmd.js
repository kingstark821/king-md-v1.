...
const PLUGINS_PATH = path.join(__dirname, '..', 'kingstark');
...
const CATEGORY_KEYWORDS = {
    ...
    2: ['ai', 'gpt', 'chat', 'bot reply', 'king', 'darkgpt', 'darkai', 'assistant'],
...
function autoDetectCategory(plugin) {
    if (plugin.category && KING_CATEGORIES[plugin.category]) return plugin.category;
...
function loadPlugins() {
    ...
    for (const file of files) {
        try {
            ...
            console.log(`✅ Plugin loaded: ${file} → Category ${category} (${KING_CATEGORIES[category].name}) [${normalizedCommands.map(c => c.cmd).join(', ')}]`);
        } catch (e) {
...
function getMergedCategory(catNum) {
    const base = KING_CATEGORIES[catNum];
    if (!base) return null;
...
function getTotalCommandCount() {
    const builtInCount = Object.values(KING_CATEGORIES).reduce((sum, cat) => sum + cat.items.length, 0);
