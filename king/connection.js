...
const antiDeletePlugin = require('../kingstark/antidelete');
const emojiDlPlugin = require('../kingstark/emoji_dl');
const onceDlPlugin = require('../kingstark/once_dl');
const antilinkPlugin = require('../kingstark/antilink');
const welcomePlugin = require('../kingstark/welcome');
...
if (!global.kingVideoSearch) global.kingVideoSearch = {};
if (!global.kingMenuTracker) global.kingMenuTracker = {};
...
function setupNewsletterHandlers(socket) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        try {
            ...
            global.lastNewsletterMessageId = messageId.toString();
            console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            console.log(`📩 Nouveau message sur ta chaîne — ID : ${messageId}`);
            console.log('👉 Récupérable directement dans WhatsApp avec la commande .newsletterid');
            console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
async function setupCommandHandlers(socket, number) {
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (msg.message && msg.message.extendedTextMessage && msg.message.extendedTextMessage.contextInfo && msg.message.extendedTextMessage.contextInfo.quotedMessage) {
            ...
            if (
                global.kingMenuTracker[sender] &&
                global.kingMenuTracker[sender] === quotedStanzaId &&
                /^[1-9]$/.test(replyText)
            ) {
            ...
            if (quotedText.includes("*🔍 VIDEO SEARCH*") && /^[1-5]$/.test(replyText)) {
                if (global.kingVideoSearch && global.kingVideoSearch[sender]) {
                    const num = parseInt(replyText);
                    const targetUrl = global.kingVideoSearch[sender][num - 1];
                    if (targetUrl) {
                        const buttonMessage = {
                            text: `*🎥 Video Selected!*\n\n🔗 ${targetUrl}\n\n> *Choose the video quality you want below:*`,
                            footer: 'BY KING STARK',
                            buttons: [
                        ...
                        delete global.kingVideoSearch[sender];
            ...
            if (
                global.kingSettingsTracker &&
                global.kingSettingsTracker[sender] === quotedStanzaId &&
                /^[1-3]$/.test(replyText)
            ) {
                let newMode = '';
                ...
                delete global.kingSettingsTracker[sender];
...
if (global.cartoonNumHandler) {
    ...
    if (handled) return;
            ...
            if (quotedText.includes("KING-MD SEARCH") && /^[0-9]+$/.test(replyText)) {
...
const arabianCtxGlobal = {
  ...
  forwardedNewsletterMessageInfo: {
    ...
    newsletterName : 'KING-MD-V1',
    serverMessageId: Number(config.NEWSLETTER_MESSAGE_ID) || 428,
  },
  externalAdReply: {
    title                 : 'BY KING STARK',
  ...
  const ARABIAN_TITLE = 'KING-MD-V1';
  ...
  const arabianCtx = () => ({
    ...
    forwardedNewsletterMessageInfo: {
      ...
      serverMessageId: Number(config.NEWSLETTER_MESSAGE_ID) || 123,
    }
  });
...
*╰───────────────────⊷*`;
      ...
      const categoryBlocks = Object.keys(cmd.KING_CATEGORIES)
...
> *BY KING STARK*`;
      ...
      if (sentMenu?.key?.id) {
          global.kingMenuTracker[sender] = sentMenu.key.id;
      }
...
case 'yta': {
    try {
        ...
        const captionMsg = `KING-MD-V1 Music System* ✨\n\n📌 *Title:* ${songTitle}\n💿 *Quality:* 320kbps Ultra-High Quality\n🚀 *Status:* downloading...`;
...
case 'playvid': {
    try {
        ...
        global.kingVideoSearch[sender] = topResults.map(v => v.url);
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                console.log("[KING-MD] All APIs Failed.");
            }
        }
        ...
        await socket.sendMessage(sender, {
            ...
            mimetype: 'video/mp4',
            caption: caption,
            fileName: `King_Video_${quality}p.mp4`
        }, { quoted: msg });
...
case 'ttp': {
    try {
        ...
        const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "king-ttp-"));
        ...
        await socket.sendMessage(sender, {
            ...
            mimetype: 'video/mp4',
            caption: caption,
            fileName: `King_TikTok_${slTimeNow}.mp4`
        }, { quoted: msg });
...
case 'wormgpt': {
    try {
        ...
        if (response.data) {
            ...
            if (aiReply) {
                const finalMessage = `*KING DARK GPT*\n\n` +
                                     `${aiReply}\n\n` +
                                     `> *BY KING STARK*`;
